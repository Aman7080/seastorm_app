...
function MintBox() {
  ...
  const contract = useSignatureDrop(contractAddress);
  const isWrongNetwork = useNetworkMismatch();
  const [, switchNetwork] = useNetwork();
  const [mintAmount, setMintAmount] = useState(1);
  const { data: contactMetadata } = useContractMetadata(contractAddress);
  const { data: claimedNFTSupply } = useClaimedNFTSupply(contract);
  const { data: unClaimedNFTSupply } = useUnclaimedNFTSupply(contract);
  const { data: ownedNFTs } = useNFTBalance(
    contract,
    "0x71E62A15b8b78656787508ed8339786dE9123fDA"
  );
  // console.log("Owned NFTS : ", ownedNFTs?.toNumber());
  const mint = async () => {
    if (!address) {
      connectMetamask;
      return;
    ...
    setClaiming(true);
    try {
      await contract?.claim(mintAmount);
  ...
  const incAmount = () => {
    if (mintAmount < 2) {
      ...
      console.log("incmintAmount = " + mintAmount);
    }
  };
  function decAmount() {
    if (mintAmount > 1) {
      setMintAmount(mintAmount - 1);
      console.log("decmintAmount = " + mintAmount);
    }
  }

  return (
    <div className="sm:w-full mx-auto flex flex-col p-6 md:p-12 ">
      {address && (
        <div className="grid gap-7 rounded-md p-6 md:grid-cols-2 md:p-12 border-double border-4">
          <div className="">
            <img src={contactMetadata?.image} className="rounded-md" />
          </div>
          ...
          <div>
            <div className=" w-full max-w-lg ">
              <div className="flex flex-col items-center justify-center space-y-6">
                ...
                <p>Mint Price : 0.02 Eth per NFT</p>
                <p>Minted : {claimedNFTSupply?.toNumber() || 0}</p>
                <p>Available : {unClaimedNFTSupply?.toNumber() || 0}</p>
              ...
              <div className="flex space-x-6 mt-10 justify-center">
                <button
                  ...
                  className="px-3 py-1 border-black border disabled:opacity-25 text-2xl hover:animate-bounce"
                >
                  <AiFillDislike />
                </button>
                <p className="font-bold">{mintAmount}</p>
                <button
                  onClick={incAmount}
              ...
              <div className="flex justify-center mt-6">
                <button
                  className=" border-4 bg-teal-400 px-6 py-2 text-white hover:text-black hover:scale-110 hover:font-bold hover:bg-green-400"
                  onClick={mint}
                  disabled={claiming}
                >
                  {claiming ? "Minting .... " : "Mint Now"}
                </button>
              </div>
              <div className="flex justify-center mt-6">
                <button
                  className=" bg-teal-400 px-6 py-2 text-white hover:text-slate-800 border-teal-900 border-solid border-4 mt-3 hover:scale-110 hover:font-bold hover:bg-orange-400"
                  onClick={disconnect}
                >
                  Disconnect
                </button>
      ...
      {!address && (
        <div className="md:flex md:space-x-5 md:justify-center text-center">
          <button
            className="border bg-teal-400 px-6 py-2 text-white hover:text-black flex "
          ...
          <button
            className="border bg-teal-400 px-6 py-2 text-white hover:text-black flex mt-5 md:mt-0 "
